...
//
void move_particles_soft(particle_array_t p, const f32 dt, u64 n) {
  //
  ...
  for (u64 i = 0; i < n; i++) {
    ...
    for (u64 j = 0; j < n; j++) {
      ...
      f32 d_3_over_2 = 1 / sqrtf(d_2);                               // 11//12
...
//
void move_particles_if(particle_array_t p, const f32 dt, u64 n) {
  //
  const f32 softening = 1e-20;
...
//
void move_particles_split(particle_array_t p, const f32 dt, u64 n) {
  //
  const f32 softening = 1e-20;
...
int main(int argc, char **argv) {
  ...
  for (u64 i = 0; i < steps; i++) {
    ...
    move_particles_soft(p, dt, n);

    const f64 end = omp_get_wtime();

    printf("%5llu %.8e %s\n", i, (end - start), (i < warmup) ? "*" : "");
    fflush(stdout);
